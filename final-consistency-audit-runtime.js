...
(()=>{
 ...
 const manifestFingerprint=(p,m)=>{if(!m)return'';const files=Array.isArray(m.files)?m.files:[];return hash(JSON.stringify({journal:p.journal,articleType:p.articleType,authors:p.authors||p.authorNames,journalRequirementsFingerprint:m.journalRequirementsFingerprint||'',files}))};
 const audit=s=>{
  ...
  add('Submission manifest fingerprint integrity',manifest&&manifest.packageFingerprint&&manifest.packageFingerprint===manifestFingerprint(p,manifest)?'verified':manifest?'unverified':'missing','The saved package fingerprint must match its current file inventory, journal, authors and journal-requirements fingerprint.');
