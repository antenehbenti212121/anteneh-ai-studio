...
(()=>{
  ...
  const load=()=>{if(window.__stageWorkspaceLoaded)return;window.__stageWorkspaceLoaded=true;
    const add=(src,fn)=>{const s=document.createElement('script');s.src=src;if(fn)s.onload=fn;document.head.appendChild(s)};
    add('/stage-workspace.js'); add('/research-decision-record.js',()=>window.ResearchDecisionRecord?.render?.()); add('/multilingual-collection.js',()=>window.MultilingualCollection?.render?.(Number(read().current||0))); add('/questionnaire-builder.js',()=>window.QuestionnaireBuilder?.render?.(Number(read().current||0))); add('/xlsform-exporter.js',()=>window.XLSFormExporter?.render?.(Number(read().current||0))); add('/data-collection-stage.js',()=>window.DataCollectionStage?.render?.(Number(read().current||0))); add('/data-quality-runtime.js',()=>window.DataQualityRuntime?.render?.(Number(read().current||0))); add('/analysis-plan-runtime.js',()=>window.AnalysisPlanRuntime?.render?.(Number(read().current||0))); add('/results-trace-runtime.js',()=>window.ResultsTraceRuntime?.render?.(Number(read().current||0))); add('/literature-synthesis-runtime.js',()=>window.LiteratureSynthesisRuntime?.render?.(Number(read().current||0))); add('/discussion-runtime.js',()=>window.DiscussionRuntime?.render?.(Number(read().current||0))); add('/reference-audit-runtime.js',()=>window.ReferenceAuditRuntime?.render?.(Number(read().current||0))); add('/reference-manager-runtime.js',()=>window.ReferenceManagerRuntime?.render?.(Number(read().current||0))); add('/analysis-readiness.js',()=>window.AnalysisReadiness?.render?.(Number(read().current||0))); add('/academic-qc-runtime.js',()=>window.AcademicQCRuntime?.render?.(Number(read().current||0))); add('/publication-readiness-runtime.js',()=>window.PublicationReadinessRuntime?.render?.(Number(read().current||0))); add('/manuscript-readiness-runtime.js',()=>window.ManuscriptReadinessRuntime?.render?.(Number(read().current||0))); add('/publication-package-runtime.js',()=>window.PublicationPackageRuntime?.render?.(Number(read().current||0))); add('/publication-export-runtime.js',()=>window.PublicationExportRuntime?.render?.(Number(read().current||0))); add('/final-publication-validator.js',()=>window.FinalPublicationValidator?.render?.(Number(read().current||0))); add('/journal-submission-runtime.js',()=>window.JournalSubmissionRuntime?.render?.(Number(read().current||0))); add('/submission-readiness-runtime.js',()=>window.SubmissionReadinessRuntime?.render?.(Number(read().current||0))); add('/submission-package-manifest-runtime.js',()=>window.SubmissionPackageManifestRuntime?.render?.(Number(read().current||0)));
  };
