(() => {
  ...
  const captionChunks = text => {const ws=words(text),size=9,out=[];for(let i=0;i<ws.length;i+=size)out.push(ws.slice(i,i+size).join(' '));return out.length?out:[''];};

  function drawDiagram(c,s,m,p,pal){const x=760,y=155,w=440;c.save();c.textAlign='center';c.fillStyle=pal.bg==='#fffdf7'?'#fff':'rgba(255,255,255,.055)';c.strokeStyle=pal.line;c.lineWidth=2;round(c,x,y,w,400,26);c.fill();c.stroke();c.fillStyle=pal.muted;c.font='800 16px system-ui';c.fillText(m.toUpperCase(),980,y+38);c.strokeStyle=pal.accent;c.fillStyle=pal.accent;c.lineWidth=5;
    ...
    else {c.fillStyle=m==='warning'?'#f59e0b':pal.accent;c.beginPath();c.arc(980,y+210,82+7*Math.sin(p*Math.PI),0,Math.PI*2);c.fill();c.fillStyle='#fff';c.font='900 20px system-ui';c.fillText(m==='warning'?'WATCH OUT':m==='example'?'REAL EXAMPLE':m==='recap'?'TAKEAWAY':'BIG IDEA',980,y+217);}c.restore();}

  function draw(c,s,i,total,p,style,caption){const pal=palette(style),w=c.canvas.width,h=c.canvas.height,m=mode(s);c.clearRect(0,0,w,h);c.fillStyle=pal.bg;c.fillRect(0,0,w,h);
    ...
    if(style==='Minimal motion graphics'){c.globalAlpha=.08;c.fillStyle=pal.accent;for(let k=0;k<7;k++){c.beginPath();c.arc(90+k*205,120+Math.sin(p*6+k)*30,45+(k%3)*18,0,Math.PI*2);c.fill();}c.globalAlpha=1;}
    ...
    const slide=(1-Math.min(1,p*5))*30;c.save();c.globalAlpha=Math.min(1,p*5);c.translate(-slide,0);c.fillStyle=pal.ink;c.font='900 50px system-ui';wrap(c,s.title,620).slice(0,2).forEach((v,n)=>c.fillText(v,55,175+n*58));c.fillStyle=pal.muted;c.font='400 22px system-ui';wrap(c,s.narration,620).slice(0,6).forEach((v,n)=>c.fillText(v,60,335+n*32));c.restore();drawDiagram(c,s,m,p,pal);
    ...
    c.fillStyle=pal.muted;c.font='500 14px system-ui';wrap(c,s.visual,1050).slice(0,1).forEach(v=>c.fillText(v,55,h-62));c.fillStyle=pal.accent;c.fillRect(55,h-36,(w-110)*((i+p)/total),5);
  }
  ...
  async function render(){
    ...
    button.disabled=true;state.textContent='Rendering';preview.hidden=true;download.hidden=true;status.textContent='Preparing captions, narration and video…';if(outputUrl)URL.revokeObjectURL(outputUrl);
    const canvas=document.createElement('canvas');canvas.width=1280;canvas.height=720;const c=canvas.getContext('2d');const stream=canvas.captureStream(24);let au={stream:null,ms:0,cleanup:()=>{},reason:'none'};try{au=await audioTrack();}catch(_){ }
    const narrationExpected=!!audioPreview?.src&&!audioPreview.hidden;
    if(narrationExpected&&!au.stream){status.textContent='Narration was found, but this browser cannot route it into the video. The video will render silently.';}
    if(au.stream)au.stream.getAudioTracks().forEach(t=>stream.addTrack(t));
    const ws=items.map(s=>Math.max(1,words(s.narration).length)),sum=ws.reduce((a,b)=>a+b,0);const durations=ws.map(x=>au.ms?Math.max(1400,au.ms*x/sum):Math.max(2200,Math.min(9000,x/2.4*1000)));const total=durations.reduce((a,b)=>a+b,0);const type=mime();if(!type){status.textContent='This browser has no supported video recording format.';button.disabled=false;state.textContent='Draft';au.cleanup();return;}
    let rec;try{rec=new MediaRecorder(stream,{mimeType:type,videoBitsPerSecond:2200000,audioBitsPerSecond:128000});}catch(_){status.textContent='This browser could not start its video encoder.';button.disabled=false;state.textContent='Draft';au.cleanup();return;}
